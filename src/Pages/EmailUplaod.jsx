...
function EmailUpload() {
  ...
  const [sname,setSname] = useState(["Extracting Policies","Retreving Data","Analyzing Situation","Writing Email","Response"]);
  ...
  async function extractText(urls) {
    ...
    fileReader.onload = async function() {
        ...
        setStage([...stage,1]);
        const response = await api.getEmail(extractedText,query);
        setStage([1,2,3,4,5]);
    ...
    <>
      <div className="mainbg w-screen h-screen bg-[#0B0A0F] flex items-center justify-center gap-[24px]">
          ...
          </div>
          {
            ...
            <div className="animebtn w-full h-[40px] bg-[#181522] rounded-full flex justify-center items-center"  onClick={()=>{if(stage.length == 5){navigate("/email",{ state:response })}}}><span className='text-[#fff] text-[12px] font-semibold'>Show Results</span></div>
